...
type HSSBlockScanner struct {
	*bitcoin.BTCBlockScanner
	hssWM *WalletManager
}

//NewBTCBlockScanner 创建区块链扫描器
func NewHSSBlockScanner(wm *WalletManager) *HSSBlockScanner {
	bs := &HSSBlockScanner{hssWM: wm}
...
func (bs *HSSBlockScanner) GetBalanceByAddress(address ...string) ([]*openwallet.Balance, error) {
