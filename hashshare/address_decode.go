...
import (
	...
	"github.com/assetsadapterstore/hashshare-adapter/hashshare_addrdec"
...
func (decoder *addressDecoder) PrivateKeyToWIF(priv []byte, isTestnet bool) (string, error) {
	...
	cfg := hashshare_addrdec.HSS_mainnetPrivateWIFCompressed
	if decoder.wm.Config.IsTestNet {
		cfg = hashshare_addrdec.HSS_testnetPrivateWIFCompressed
	}

	wif, _ := hashshare_addrdec.Default.AddressEncode(priv, cfg)
...
func (decoder *addressDecoder) PublicKeyToAddress(pub []byte, isTestnet bool) (string, error) {
	hashshare_addrdec.Default.IsTestNet = decoder.wm.Config.IsTestNet
	address, err := hashshare_addrdec.Default.AddressEncode(pub)
	if err != nil {
		return "", err
	}
...
func (decoder *addressDecoder) RedeemScriptToAddress(pubs [][]byte, required uint64, isTestnet bool) (string, error) {
	...
	cfg := hashshare_addrdec.HSS_mainnetAddressP2SH
	if decoder.wm.Config.IsTestNet {
		cfg = hashshare_addrdec.HSS_testnetAddressP2SH
	}
	...
	address, _ := hashshare_addrdec.Default.AddressEncode(pkHash, cfg)
...
func (decoder *addressDecoder) WIFToPrivateKey(wif string, isTestnet bool) ([]byte, error) {
	...
	priv, err := hashshare_addrdec.Default.AddressDecode(wif, cfg)
	if err != nil {
		return nil, err
	}
